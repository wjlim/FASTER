...
def process_str_analysis(args):
    ...
    try:
        ...
        for sample_name in data['Sample File Name'].unique():
            ...
            # Detect contamination and store join points
            ...
            for marker, peaks in peaks_by_marker.items():
                ...
                if contamination_result:
                    ...
                    contamination_by_marker[marker] = contamination_info # Store only contamination info
                    if join_points: # Store join points separately
                        join_points_by_marker[marker] = join_points
            
            # Generate and save results
            results = result_generator.generate_results(
                peaks_by_marker=peaks_by_marker,
                # Pass contamination_info dictionary
            ...
            # Generate plots
            if args.plot:
                # plot_sample_summary now needs join_points
                plotter.plot_sample_summary(
                    peaks_by_marker,
                    contamination_by_marker,
                    sample_name,
                    str(plot_dir),
                    join_points_by_marker # Pass join points here
                )
            
            # Generate plotly plots for HTML report
            if args.plotly:
                # generate_plotly_plots now needs join_points
