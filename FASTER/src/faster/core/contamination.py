...
class ContaminationDetector:
    ...
    def _neighbor_joining(self, peaks: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, List[JoinPoint], Optional[float]]:
        ...
        # Process remaining peaks sequentially
        for i in range(1, len(peaks)):
            ...
            # Handle potential division by zero if reference height is 0
            current_ref_height = current_ref_peak['height']
            if current_ref_height == 0:
                relative_distance = float('inf') # Treat as infinite distance
            else:
            ...
            if relative_distance <= self.RELATIVE_DISTANCE_THRESHOLD and len(main_peaks_list) < self.MAX_MAIN_PEAKS:
                ...
                # Calculate joining point between previous reference and current peak
                join_x, join_y = self._find_joining_point(current_ref_peak, next_peak)
                join_points.append(JoinPoint(
                    size=join_x,
                    height=join_y,
                    joined_peaks=[current_ref_idx, i] # Store indices of joined peaks
                ))
                # Update reference index for the next iteration
                current_ref_idx = i 
            else:
                # Break if distance threshold exceeded or max peaks reached
                triggering_distance = relative_distance # Store the distance that caused the break
                break
        
        # Convert main peaks list to DataFrame
        main_peaks_df = pd.DataFrame(main_peaks_list)
        
        # All remaining peaks (from index i onwards) are contamination
